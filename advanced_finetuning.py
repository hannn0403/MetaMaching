...
def kshot_fine_tuning(model, kshot_df, kshot_pheno, max_cod_idx, fine_tuned_model_pth, generator, device, seed, data_file_name): 
    ...
    pred = pd.DataFrame({'prediction': dnn_val_outputs[:, 0], 'Age':kshot_val_age})
    dnn_k_val_age_cod = get_cod_score(pred)
    ...
    while epoch <= 200: 
        ...
        if val_loss < best_val_loss : 
            ...
            pred = pd.DataFrame({'prediction': val_preds[0], 'Age':kshot_val_age})
            k_val_age_cod = get_cod_score(pred)
    ...
    return dnn_k_val_age_cod, k_val_age_cod
...
def advanced_finetuning(df, pheno_with_age, k_num_list, data_file_name, batch_size=128, iteration=10, only_test=False):
    ...
    if not only_test:
        ...
        for seed in range(1, iteration+1): 
            ...
            for k_num in k_num_list: 
                ...
                #kshot_corr, kshot_cod = test_finetuned_model(basic_model_pth, kshot_df, kshot_pheno, max_cod_idx, max_cod_list, generator, seed)
                kshot_corr, kshot_cod = test_finetuned_model(basic_model_pth, kshot_df, kshot_pheno, max_cod_idx, generator, seed)
                ...
                age_corr, age_cod = test_finetuned_model(fine_tuned_model_pth, test_df, test_pheno, max_cod_idx, generator, seed)
    ...
    return test_age_corr_10, test_age_cod_10, test_age_corr_30, test_age_cod_30, test_age_corr_50, test_age_cod_50, test_age_corr_100, test_age_cod_100, ten_shot_idx[1:], thirty_shot_idx[1:], fifty_shot_idx[1:], hund_shot_idx[1:], dnn_kshot_cods, adft_kshot_cods
